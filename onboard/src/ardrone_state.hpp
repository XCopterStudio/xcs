...
class ardroneState{
	uint32_t bitFieldStates;
public:	
	ardroneState() : bitFieldStates(0) {};
	void updateState(uint32_t bitFieldStates){this->bitFieldStates = bitFieldStates;};
	bool getState(ardronePossibleStates state){ return state & bitFieldStates ? true : false;};
};
